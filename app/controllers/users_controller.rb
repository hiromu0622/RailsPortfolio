class UsersController < ApplicationController
  ...
  def create
    ...
    if @user.save
      log_in @user
      redirect_to users_mypage_path, success: '登録が完了しました'
    else
  ...
  def area
    if params[:user_id].present?
      ...
      @prefecture = Topic.where(user_id: params[:user_id]).select(:prefecture).distinct.pluck(:prefecture)
      prefecture = [
